...
class Container extends Model
{
    ...
    static function createExcel($containers)
    {
        //create archive
        $zip = new ZipArchive;
        $archive_temp_name = tempnam(sys_get_temp_dir(), "archive_" . date('d-m-Y') . ".zip");


        //create a spreadsheet for each container
        if ($zip->open($archive_temp_name, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
            foreach ($containers as $container) {
                ...
                $writer->save($path);
                $zip->addFile($path, 'container_' . $container->number.'.xlsx');
            }
        }

        $zip->close();

        return $archive_temp_name;
    }
}
